...
function DisplayTemplate_ManagedImageHoverPanel(ctx) {ULSWtC:;
 ...
 var releaseForm = ctx.CurrentItem.ReleasedOWSBOOL;
 if(releaseForm == undefined || releaseForm == null) {
  	releaseForm = "No";
  }
  else {
    releaseForm = "Yes";
  ...
  if (showRelatedContent){
      relatedHtml = String.format('<div class="relatedsearch">' +
                                      '<div class="relatedtitle">Similar Owner </div>'+ 
                                      '<div><ul class="relatedresults" id="{0}_DocumentOwner"></ul></div>' + 
                                  '</div>' + 
    ...
    htmlMarkup += String.format('<div class="ms-srch-hover-imageContainer">' +
 		...
 		htmlMarkup += String.format('<div class="moredetails">' +
                                  '<div class="itemdetail"><div class="detailsitem detailslabel">Sector: </div><div>{4}</div></div><div class="itemdetail"><div class="detailsitem detailslabel">Last Updated: </div><div>{6}</div></div><div class="itemdetail"><div class="detailsitem detailslabel">Owner: </div><div>{1}</div></div><div class="itemdetail"><div class="detailsitem detailslabel">Release Form: </div><div>{9}</div></div><div class="itemdetail"><div class="detailsitem detailslabel">Keywords: </div><div style="overflow:hidden">{7}</div></div></div>', title, docowner, reviewdate, docRef, organisation, author, parseDateFromUTC(modifiedDate), keywords, imageuse, releaseForm);//9
	...
	AddPostRenderCallback(ctx, function(){
	    ...
	    if (showRelatedContent){
	      if (imageuse != null) {
	      	getRelatedContent(relatedId, 'DocumentOwner', docowner, title, sitePath);
	      }

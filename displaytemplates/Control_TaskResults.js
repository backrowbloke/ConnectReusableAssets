function ULSKer(){var o=new Object;o.ULSTeamName="Search Server";o.ULSFileName="Control_TaskResults.js";return o;}

function processTask(id) {
	var taskId = id.replace('chk','task');
	var cbChecked = jQuery('#' + id).is(':checked');
	
	var itempath = jQuery('#' + taskId + ' a').attr('href');
	var listid = jQuery('#' + taskId + ' a').attr('id');
	var type = 'progress';
	
	if (cbChecked) {
		jQuery('#' + id).prop('title','Mark incomplete');
		jQuery('#' + taskId + '').addClass('completedtask');
		var tasksToHide = [];
		if (localStorage.babcockTasks != undefined) {
			tasksToHide = JSON.parse(localStorage.babcockTasks);
		}
		tasksToHide.push(taskId);
		localStorage.babcockTasks = JSON.stringify(tasksToHide);
		type = 'complete';
		
	}
	else {
		jQuery('#' + id).prop('title','Mark complete');
		jQuery('#' + taskId + '').removeClass('completedtask');
		if (localStorage.babcockTasks != undefined) {
				tasksToHide = JSON.parse(localStorage.babcockTasks);
				tasksToHide.splice(tasksToHide.indexOf(taskId),1);
				localStorage.babcockTasks = JSON.stringify(tasksToHide);
				
		}
	}
	updateTask(itempath, listid, type);

}

function markTask(item, index) {
	//jQuery('#' + item + '').toggleClass('completedtask');
	//var cbId = item.replace('task','chck');
	//jQuery('#' + cbId + '').attr('checked', true);
	var tasksToHide = [];
		if (localStorage.babcockTasks != undefined) {
			tasksToHide = JSON.parse(localStorage.babcockTasks);
		}
		tasksToHide.push(item);
		localStorage.babcockTasks = JSON.stringify(tasksToHide);
}

function updateTask(path, listid, type) {
	var siteurl = path.substring(0,path.toLowerCase().indexOf('/lists'));
	var itemId = path.substring(path.indexOf('=')+1);
	...
	if (type == 'progress') {
		...
		listItem.set_item('PercentComplete',0);
	}
	listItem.update();
...
function DisplayTemplate_ControlTaskResults(ctx) {ULSKer:;
  var ms_outHtml=[];
  var cachePreviousTemplateData = ctx['DisplayTemplateData'];
  ctx['DisplayTemplateData'] = new Object();
  DisplayTemplate_ControlTaskResults.DisplayTemplateData = ctx['DisplayTemplateData'];

  ctx['DisplayTemplateData']['TemplateUrl']='~sitecollection\u002f_catalogs\u002fmasterpage\u002fDisplay Templates\u002fContentManagement\u002fControl_TaskResults.js';
...
ms_outHtml.push('<style>.completedtask{text-decoration:line-through;}</style>');
...
//,'<table class="ms-listviewtable"><tr><th class="ms-vh2"><input type="checkbox" id="chkalltasks" title="Mark all complete"/></th><th class="ms-vh2">Task Name</th><th class="ms-vh2">Due Date</th></tr>'
,'<table class="ms-listviewtable"><tr><th class="ms-vh2"><div class="ms-chkmark-container-centerer"><span class="ms-cui-img-16by16 ms-cui-img-cont-float" unselectable="on"><img class="ms-chkmark-marktaskcomplete" src="/_layouts/15/images/spcommon.png?rev=23"></span></div></th><th class="ms-vh2">Task Name</th><th class="ms-vh2">Due Date</th></tr>'
...
ms_outHtml.push(''
,'                        <tr><td colspan="3"><div id="NoResult">'
);
...
,'                                <ul>'
,'                                    <li>No incomplete tasks found assigned to you</li>'
...
,'    '
	...
	AddPostRenderCallback(ctx, function(){
		jQuery('#chkalltasks').change(function() {
			if(jQuery(this).prop('checked')){
				//clear check box to enable click to set
				jQuery('.babcocktaskcheck').prop('checked', false);
				jQuery(this).prop('title', 'Mark all incomplete');
			}
			else {
				//set check box to enable click to clear
				jQuery('.babcocktaskcheck').prop('checked', true);
				jQuery(this).prop('title', 'Mark all complete');
			}
			jQuery('.babcocktaskcheck').trigger('click');
			
		});
		jQuery('.babcocktaskcheck').change(function() {
			processTask(jQuery(this).prop('id'));
		});
		
		var tasksToHide = [];
		if (localStorage.babcockTasks != undefined) {
			tasksToHide = JSON.parse(localStorage.babcockTasks);
			localStorage.babcockTasks = JSON.stringify([]);;
			tasksToHide.forEach(markTask);			
		}
	});
...
if ("undefined" != typeof (Srch) &&"undefined" != typeof (Srch.U) &&typeof(Srch.U.registerRenderTemplateByName) == "function") {
  Srch.U.registerRenderTemplateByName("Control_TaskResults", DisplayTemplate_ControlTaskResults);
}

if ("undefined" != typeof (Srch) &&"undefined" != typeof (Srch.U) &&typeof(Srch.U.registerRenderTemplateByName) == "function") {
  Srch.U.registerRenderTemplateByName("~sitecollection\u002f_catalogs\u002fmasterpage\u002fDisplay Templates\u002fContentManagement\u002fControl_TaskResults.js", DisplayTemplate_ControlTaskResults);
}

}
RegisterTemplate_a1f56356c3df489c90ede20cd42493a5();
if (typeof(RegisterModuleInit) == "function" && typeof(Srch.U.replaceUrlTokens) == "function") {
  RegisterModuleInit(Srch.U.replaceUrlTokens("~sitecollection\u002f_catalogs\u002fmasterpage\u002fDisplay Templates\u002fContentManagement\u002fControl_TaskResults.js"), RegisterTemplate_a1f56356c3df489c90ede20cd42493a5);
}
